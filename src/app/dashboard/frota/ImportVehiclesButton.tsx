...
function ImportButton({ onClick, fileName }: { onClick: () => void; fileName: string }) {
  ...
  return (
    <Button
      ...
      disabled={pending}
      onClick={onClick}
      aria-label="Importar veículos de arquivo CSV, CDSV ou TXT"
      title="Importar veículos"
    >
      {pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileUp className="h-4 w-4" />}
      {pending ? 'Importando...' : fileName || 'Importar veículos'}
    </Button>
  )
}
...
export default function ImportVehiclesButton() {
  ...
  const [state, action] = useFormState(importVehicles, initialState)
